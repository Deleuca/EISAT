d3.json("graph.json").then(function (data) {
	...
	nodeGroup.append("circle")
		...
		.on("auxclick", (event, d) => handleMiddleClick(event, d)); // Attach auxclick to circle
	...
	function handleMiddleClick(event, d) {
		...
		if (!pendingMiddleNode) {
			...
			if (pendingMiddleNode.id !== d.id) {
				const eK = edgeKey(pendingMiddleNode.id, d.id); // Ensure edgeKey returns a consistent string
